...
*/
  ...
  {
    PetscInt    ilen_row,*imat_ilen,Crow;
    /* PetscScalar *imat_a; */
    mat       = (Mat_SeqAIJ*)submat->data;
    imat_ilen = mat->ilen;
    /* PetscInt *imat_j    = mat->j; */
    /* imat_i    = mat->i; */
    /*    imat_a    = mat->a; */
  ...
  if (scall == MAT_INITIAL_MATRIX) {
    ...
    if (!allcolumns) {
      for (j=0; j<nrow; j++) {
        ...
        mat_a = imat_a + mat_i;
        mat_j = imat_j + mat_i;
        ierr  = PetscSortIntWithMatScalarArray(imat_ilen[j],mat_j,mat_a);CHKERRQ(ierr);
      }
    }
  }
#endif
...
PetscErrorCode MatGetSubMatrices_MPIAIJ(Mat C,PetscInt ismax,const IS isrow[],const IS iscol[],MatReuse scall,Mat *submat[])
{
  ...
  */
  ...
  printf("ismax %d\n",ismax);
  PetscMPIInt tismax;
  ierr = MPIU_Allreduce(&ismax,&tismax,0,MPIU_INT,MPI_MAX,PetscObjectComm((PetscObject)C));CHKERRQ(ierr);
